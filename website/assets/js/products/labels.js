/**
 * Labels JavaScript | Cannlytics Website
...
export const labelsJS = {
  ...
  // renderLabelPlaceholder: renderLabelPlaceholder,

  initializeLabelParser() {
    /* Initialize the label parser UI. */

    // Wire-up file import functions.
    document.getElementById('label-input').onchange = function() {
      cannlytics.labels.uploadLabel();
    }
    document.getElementById('label-import-button').onclick = function() {
    ...
    this.initializeDragAndDrop(cannlytics.coas.uploadLabel);
  ...
  initializeDragAndDrop(uploadCallback) {
    ...
    $(document).on('dragenter', '.box__dragbox', function (e) {
        e.stopPropagation();
        e.preventDefault();
        document.getElementById('dropbox-text').innerHTML = `Drop your labels here.`;
    ...
    $(document).on('dragleave', '.box__dragbox', function (e) {
        e.stopPropagation();
        e.preventDefault();
        document.getElementById('dropbox-text').innerHTML = `Drop a label to parse.`;
  ...
  uploadLabel(formData, formId = 'label-import-form') {
    /* Upload a label. */
    ...
    const successCallback = this.renderLabelData;
  ...
  async postCoAData(data) {
    ...
    if (response.success) {
      this.renderLabelData(response.data);
    } else {
...
async function downloadCoAData(data) {
  ...
  try {
    const response = await authRequest('/api/coas/download', postData, { file: true });
...
function renderLabelPlaceholder() {
  /**
